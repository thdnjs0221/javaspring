...
/**
 * 예외 
 *		: 프로그램 정상 실행이 중단될 수 있는 모든 예외적인 상황이나 조건에 대한 표현.
 *java.lang.Throwable : throw/catch 가능한 모든 예외의 최상위
 * - java.lang.Error : 시스템 실패 상황으로 개발자가 직접 처리할 수 없는 에러
 * - java.lang.Exception : 어떤 상황을 표현하기 위해 의도적으로 throw 되는 예외의 통칭, 필요한 경우, catch로 처리 가능
 *		 - checked exception(non RuntimeException) : 발생 가능한 코드 블럭내에서 어떤 형태로 해당 예외를 처리해야 하고,
 *														처리하지 않으면, 컴파일 에러가 발생함. 
 *		 - unChecked exception : RuntimeException의 하위 예외들로 주로 JVM이 발생시키고,
 *									throws 키워드가 없어도 기본으로 throw 되고 있는 예외들.
 *									직접 catch 하지 않으면 , 호출자에게 전달되고, 최종적으로JVM에게 전달됨.
 *
 *예외 처리 방법
 *- 예외 회피(throws) : 메소드 콜스택에 저장된 복귀 주소에 따라 예외의 처리를 호출자에게 떠넘기는 전략
 *- 직접처리 : try(Close 객체 생성)~catch(exception)~finally
 *
 *try{
 *
 *		//잠재적 예외 발생 코드
 ...
 *
 *		//직접 처리 
 *		- 예외 전환 : 구체적인 상황을 명시할 수 있는 예외, 혹은 checked를 unChecked 예외로 변경 -> wrapping(원본예외 유지)
 *				ex)
 *				catch(IOExcepton e){
...
class ExceptionDesc {
	...
	int unCheckedExceptionGenerate()   {
		...
		return 10+checkedExceptionGenerate();
			...
			throw new CustomException(e);  //CustomException Checked exception  이기 떄문에 RuntimeException으로 변경
		}
	}
