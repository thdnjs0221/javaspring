...
<script>
		...
		connect();
		function connect() {
			webSocket = new WebSocket("ws://192.168.35.82:9080/socket/merong"); // End Point
			//이벤트에 이벤트핸들러 뜽록 
		...
		function fMessage() {
			let data = event.data;
			console.log("누느로 체킁:",data);
			let msgVO = JSON.parse(data);
			//alert(`\${msgVO.from}님에게서 \${msgVO.msg} 연락이 왔어용`);

			c_chatWin.innerHTML = c_chatWin.innerHTML + 
			        `\${msgVO.from}님에게서 \${msgVO.msg} 연락이 왔어용`;
		}
